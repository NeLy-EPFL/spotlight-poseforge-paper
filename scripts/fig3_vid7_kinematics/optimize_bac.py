...
def get_adjusted_grf(sim_results, adhesion_force_per_leg, steps_offset):
    """GRF z-component (T_kin, n_legs) at kinematic rate, with per-leg adhesion subtracted.
    ...
    """
    # z-component, negated: upward GRF. Shape: (T_sim, n_legs)
    grf_ts = sim_results["ground_contacts"]["forces_world"][:, :, 2] * -1
...
def compute_contact_error(
    sim_results, gait_info, adhesion_force_per_leg, steps_offset, threshold
):
    ...
    """
    grf_ts = get_adjusted_grf(sim_results, adhesion_force_per_leg, steps_offset)
...
def run_optimization(
    ...
    """Run multi-objective Optuna study minimizing trajectory error and contact error.

    Two objectives:
        obj1 — weighted MSE between recorded and simulated linear speed & turn rate.
        obj2 — 1 - IOU between GRF-derived contact mask and kinematic stance mask.

    Five parameters are optimized:
        actuator_gain, joint_damping, sliding_friction,
        adhforce_fmlegs (fore/mid legs), adhforce_hlegs_scale (hind leg scale factor).
    """
    ...
    def objective(trial):
        ...
        # ── Objective 1: trajectory error ──
        # align_smooth_decompose_trajs handles t_range subselection internally
        ...
        # ── Objective 2: contact pattern error ──
        # get_gait_info handles t_range subselection internally
    ...
    study = optuna.create_study(
        ...
        study_name="nmf_physics_optim",
    )
    study.optimize(objective, n_trials=n_trials, show_progress_bar=True)

    # Persist the study object for later analysis / Pareto front plotting
    with open(output_basedir / "optuna_study.pkl", "wb") as f:
        pickle.dump(study, f)
...
if __name__ == "__main__":
    ...
    # ── Debug test run (replay_walking.py parameters) ─────────────────────────
    # Create a manager with the same LEG_ADHESION_GAIN as replay_walking.py so
    # that the test run is a faithful reproduction of that script's behavior.
    print("\nRunning debug test with replay_walking.py parameters...")
    ...
    # Effective per-leg adhesion for GRF subtraction = gain * scalar force
    ...
    make_debug_plots(
        ...
        sim_results=test_sim_results,
        gait_info=test_gait_info,
        ...
        steps_offset=test_steps_offset,
    )

    # ── Quick sanity-check metrics for the test run ──
    ...
    print(f"\nTest-run objective values (for reference):")
    print(f"  Trajectory error : {test_traj_err:.4f}")
    print(f"  Contact error    : {test_contact_err:.4f}")

    # ── Wait for manual inspection before starting optimization ──
    # print(
    #     "\nPlease inspect the debug outputs in:\n"
    #     f"  {debug_dir}\n"
    #     "Then press Enter to start the optimization, or Ctrl+C to abort."
    # )
    # input()

    # # ── Build the replay manager for optimization ─────────────────────────────
    # # No leg_adhesion_gain here — adhesion is parameterized entirely through the
    # # per-leg dict passed to create_sim in the objective function.
